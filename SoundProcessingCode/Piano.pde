...
class PianoInstrument implements Instrument {
  ...
  PianoInstrument(float midiNote, float velocity, float masterVol) {
    ...
    */
    ...
    // 4. ハンマーの打撃ノイズ（アタック）
    // ==========================================
    float hammerVol = masterVol * 0.007f * normVel; 
    ...
    hammerEnv = new ADSR(1.0f, 0.01f, 0.06f, 0.0f, 0.01f);
  ...
  void noteOn(float dur) {
    ...
    masterEnv.patch(out); 
    hammerEnv.patch(out); // ハンマーノイズは独立して出力
  }
  ...
  void noteOff() {
    ...
    hammerEnv.unpatchAfterRelease(out);
  }
}
