...
void setup()
{
  ...
  longOut = new Summer();
  AddReverb longReverb = new AddReverb(16000, 0.20f);
...
void serialEvent(Serial p) {
  ...
  switch (header) {
    ...
    case "N" :
      ...
      for (int i = 4; i <= listLength; i += 4) {
        ...
        try {
          ...
          endVel   = float(list[i]);
          println("N受信(" + pitch + ", " + duration + ", " + startVel + ", " + endVel + ")"); 
        ...
        switch (arduinoNumber) {
          ...
          case 4 :
            ...
            else if (a == 3) player.PlaySnare2(startVel, currentMasterVolume);
            else if (a == 4) player.PlayHiHat2(startVel, currentMasterVolume);
            break;
          default: break;
        }
      }
    ...
    case "V" :
      ...
      currentMasterVolume = map( constrain( getVolume, minimumVolume, maximumVolume), minimumVolume, maximumVolume, 0.5, 1.0);
      break;
...
void Play()
{
  switch(timbre){
    ...
    case "Piano":
      //int a = millis();
      player.PlayPiano(soundOctave + soundDoReMi, 3.0f, soundStartVelocity, 1.0f);
      //a = millis()-a;
      //println("経過時間: " + a);
      break;
  }
...
void keyPressed() {
  ...
  case 'B':
    ...
    case 'F' : // トルコ行進曲
      tick = 8280;
      returnTick = new int[][] {{8664, 8280, 1399}, {9432, 8664, 1499}, {9816, 9432, 1669}, {10200, 9816, 1783}, {10968, 10200, 1895}, {10968, 9432, 1669}, {9816, 9432, 1669}, {9816, 8280, 1399}, {8664, 8280, 1399}, {9432, 10968, 2111}, {11352, 10968, 2111}, {11328, 11376, 2225}, {12960, 8280, 1399}};
      returnIndex = 0;
      noteIndex = 1399;
      interval = GetInterval( 120 );
      lastTime = System.nanoTime();
