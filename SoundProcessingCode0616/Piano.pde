...
class PianoInstrument implements Instrument {
  ...
  PianoInstrument(float midiNote, float velocity, float masterVol) {
    ...
    masterEnv = new ADSR(1.0f, 0.001f, 1.5f, 0.3f, 0.2f);
    mixer.patch(masterEnv); 
    
    //float f0 = Frequency.ofMidiNote(midiNote).asHz();
    float normVel = constrain(velocity, 0, 127) / 127.0f;
    float volFactor = masterVol * 0.024f; 
    
    //volFactor *= (0.5 + (midiNote/254.0f));  // 必要かどうか検討
    ...
    */
    ...
    // 4. ハンマーの打撃ノイズ（アタック）
    // ==========================================
    float hammerVol = masterVol * 0.002f * normVel; 
    ...
    hammerEnv = new ADSR(1.0f, 0.01f, 0.04f, 0.0f, 0.01f);
  ...
  void noteOn(float dur) {
    ...
    hammerEnv.patch(mediumOut); // ハンマーノイズは独立して出力
  }
  ...
  void noteOff() {
    ...
    hammerEnv.unpatchAfterRelease(mediumOut);
  }
}
