...
class StringsInstrument implements Instrument {
  ...
  */
  ...
  Line[] wobbleLines = new Line[MAX_TOTAL_OSCILS]; // 【新規追加】ウナリのフェード用ライン 6/12
  ...
  StringsInstrument(float midiNote, float duration, float startVel, float endVel, float masterVol) {
    ...
    // =========================================================
    float attackTime = 0.8f; // ロングトーン時の自然で豊かな立ち上がり時間
    float attackTimeSetting = constrain(startVel, 0, 127) / 127.0f * 0.7;
    attackTime -= attackTimeSetting;
    if (duration < 1.0f - attackTimeSetting) {
      attackTime = duration - 0.1f;
      if (attackTime < 0.02f) {
        attackTime = 0.02f; // 0秒によるプチプチノイズ（ポップノイズ）を防ぐセーフティ
      }
    }
    
    masterEnv = new ADSR(1.0f, attackTime, 0.5f, 0.9f, 0.15f);
    mixer.patch(masterEnv); 
    
    //float f0 = Frequency.ofMidiNote(midiNote).asHz();
    float volFactor = masterVol * 0.025f; 
    ...
    // =========================================================
    float baseVibSpeed = map(midiNote, 48, 100, 5.2f, 6.5f)*1.5;
    ...
    // =========================================================
    float fmDepthCents = map(midiNote, 48, 100, 10.0f, 25.0f)*0.5;
    ...
    */
    ...
    float biteVol = masterVol * 0.004f * startNorm; 
  ...
  */
  ...
  private void createInstrumentOscillators(final float[][] profile, float blendRatio, int maxPeaks, float volFactor, float globalVibSpeed, float fmDepthCents, Summer mixer, float startGainFactor, float endGainFactor) {
    ...
    for (int h = 0; h < limitPeaks; h++) {
      ...
      //if (targetFreq > 22050.0f) continue;
      if (targetFreq > 13000.0f) continue;
      
      // 6/13【新設：数学的倍音フィルター】
      // 8000Hzから18000Hzにかけて、その倍音の振幅（パワー）を滑らかに 100% から 0% へ減衰させる
      if (targetFreq > 7000.0f) {
        float filterFactor = map(targetFreq, 7000.0f, 15000.0f, 1.0f, 0.0f);
  ...
  */
  ...
  void noteOn(float dur) {
    ...
    for(int i = 0; i < totalActiveOscils; i++) {
      ...
      float wobbleDepth = map(i, 0, MAX_TOTAL_OSCILS - 1, 0.15f, 0.60f);
  ...
  void noteOff() {
    ...
    biteEnv.unpatchAfterRelease(mediumOut);
  }
}
