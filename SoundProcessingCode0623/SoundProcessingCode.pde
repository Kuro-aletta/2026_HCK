...
void setup()
{
  ...
  longOut = new Summer();
  AddReverb longReverb = new AddReverb(18000, 0.21f);
  ...
  for (int i = 0; i < Note_f.length; i++) {
    for (int j = 0; j < Note_f[i].length; j++) {
      Note[i][j] = int(Note_f[i][j]); // 小数点以下は切り捨てられます
    }
  }
...
void Play()
{
  switch(timbre){
    case "Flute":
      player.PlayFlute(soundOctave + soundDoReMi, 4.0f, soundStartVelocity, soundEndVelocity, 1.0f);
      break;
    case "Flure2":
      player.PlayFlute(soundOctave + soundDoReMi + 128, 4.0f, soundStartVelocity, soundEndVelocity, 1.0f);
      break;
    case "Strings":
      player.PlayStrings(soundOctave + soundDoReMi, 3.0f, soundStartVelocity, soundEndVelocity, 1.0f);
      break;
    case "Drum":
      if (soundDoReMi == 0) {
