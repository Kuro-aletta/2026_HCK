...
class FluteInstrument implements Instrument {
  ...
  FluteInstrument(float midiNote, float duration, float startVel, float endVel, float masterVol) {
    ...
    float masterAttackTime = 0.25f - constrain(startVel, 0, 127) / 127.0f * 0.2;
    masterAttackTime = min(masterAttackTime, duration - 0.1f);
    if (masterAttackTime < 0.04f) masterAttackTime = 0.04f;
    ...
    masterEnv = new ADSR(1.0f, masterAttackTime, 0.2f, 0.9f, 0.5f);
    ...
    float attackNoiseVol = masterVol * 0.02f * pow(startNorm, 1.1f); 
    ...
    startSustainVol = masterVol * 0.01f * pow(startNorm, 1.1f);
    endSustainVol   = masterVol * 0.01f * pow(endNorm, 1.1f);
